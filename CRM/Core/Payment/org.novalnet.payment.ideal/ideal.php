...
#########################################################
  ...
  /*
   * Payment Processor class for Novalnet iDEAL
   */
class org_novalnet_payment_ideal extends CRM_Core_Payment {
  ...
  function __construct($mode, &$paymentProcessor) {
    ...
    $this->_processorName     = ts('Novalnet iDEAL');
    $this->_paymentType       = 'novalnet_ideal';
    ...
    $template->assign('novalnet_ideal_name', trim($this->_paymentProcessor['name']));
    $template->assign('novalnet_ideal_logo', $logo);
    $template->assign('novalnet_ideal_desc', $description);
  }
  ...
  static function &singleton($mode, &$paymentProcessor, &$paymentForm = NULL, $force = FALSE) {
      ...
      if (self::$_singleton[$processorName] === NULL ) {
          self::$_singleton[$processorName] = new org_novalnet_payment_ideal($mode, $paymentProcessor);
      }
...
/**
   ...
   */
   function doTransferCheckout(&$params, $component) {
     ...
     CRM_Core_Payment_novalnet::getPaymentReference($data, 'nn_ideal');
