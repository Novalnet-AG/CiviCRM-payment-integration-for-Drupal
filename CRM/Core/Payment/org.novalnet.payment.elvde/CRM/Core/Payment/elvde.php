...
class CRM_Core_Payment_elvde extends CRM_Core_Payment {
  ...
  function __construct($mode, &$paymentProcessor) {
    ...
    $this->_processorName     = ts('Novalnet Direct Debit German');
    ...
    $this->_manualchecklimit  = (isset($config->nn_de_manualamount))?trim($config->nn_de_manualamount):'';
    $this->_productid2        = (isset($config->nn_de_manualamount))?trim($config->nn_de_productid2):'';
    $this->_tariffid2         = (isset($config->nn_de_manualamount))?trim($config->nn_de_tariffid2):'';
    $this->_paymentType       = 'novalnet_elv_de';
    list($logo, $description) = CRM_Core_Payment_novalnet::assignLogoAndDescription();
    $template->assign('novalnet_elvde_name', trim($this->_paymentProcessor['name']));
    $template->assign('novalnet_elvde_logo', $logo);
    $template->assign('novalnet_elvde_desc', $description);
  ...
  static function &singleton($mode, &$paymentProcessor, &$paymentForm = NULL, $force = FALSE) {
      ...
      if (self::$_singleton[$processorName] === NULL ) {
          self::$_singleton[$processorName] = new CRM_Core_Payment_elvde($mode, $paymentProcessor);
      }
 ...
 function doTransferCheckout(&$params, $component) {
    ...
    $_SESSION['elvde']['account_number'] = CRM_Core_Payment_novalnet::sanitizeNumber($_SESSION['elvde']['account_number']);
    $_SESSION['elvde']['bank_code']      = CRM_Core_Payment_novalnet::sanitizeNumber($_SESSION['elvde']['bank_code']);
    CRM_Core_Payment_novalnet::formValidation( $_SESSION['elvde'] ,$this->_paymentProcessor, $error);
    if(isset($error['form']) && $error['form']!='') {
      CRM_Core_Payment_novalnet::paymentUpdation($response);
      CRM_Core_Error::statusBounce($error['form'], $cancelURL);
    }
    if(isset($error['acdc']) && $error['acdc']!='') {
      CRM_Core_Payment_novalnet::paymentUpdation($response);
        CRM_Core_Error::statusBounce($error['acdc'], $cancelURL);
    }
    ...
    $data['bank_account_holder']  = $_SESSION['elvde']['account_holder'];
    $data['bank_account']         = $_SESSION['elvde']['account_number'];
    $data['bank_code' ]           = $_SESSION ['elvde']['bank_code'];
    $data['check']                = ($this->_mode == 'live')?'live':'test';
    $data['lang']                 = $language;
    $data['language']             = $language;
     
    CRM_Core_Payment_novalnet::urlparams($params, $data);
    CRM_Core_Payment_novalnet::getPaymentReference($data, 'nn_de');
    if (isset($_SESSION['elvde']['acdc_control']) && $_SESSION['elvde']['acdc_control'] == 1) {
      $data['acdc'] = 1;
    }
    ...
    if (isset($parsed['status']) && $parsed['status'] == '100') {
      ...
      CRM_Core_Payment_novalnet::orderCompleteParam($component, $params, $final, $data) ;
