...
class CRM_Core_Payment_paypal extends CRM_Core_Payment {
    ...
    function __construct($mode, &$paymentProcessor) {
        ...
        defined('MODE_PAYPAL') || define('MODE_PAYPAL', $this->_testmode);
        $this->_notify_url = trim(Civi::settings()->get('nn_notify_url'));
        $this->_manualchecklimit = trim(Civi::settings()->get('nn_paypal_manualamount'));
    ...
    public function handlePaymentNotification() {
        ...
        if (isset($response['hash2']) && (in_array($response['status'], array(90, 100)))) {
