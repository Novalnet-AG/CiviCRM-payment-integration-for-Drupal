 ...
 */
class CRM_Core_Payment_ideal extends CRM_Core_Payment {
    ...
    function __construct($mode, &$paymentProcessor) {
        ...
        $this->_testmode = trim(Civi::settings()->get('nn_ideal_testmode'));
        defined('MODE_IDEAL') || define('MODE_IDEAL', $this->_testmode);
    ...
    static function &singleton($mode, &$paymentProcessor, &$paymentForm = NULL, $force = FALSE) {
        ...
        if (self::$_singleton[$processorName] === NULL) {
            self::$_singleton[$processorName] = new CRM_Core_Payment_ideal($mode, $paymentProcessor);
        }
    ...
    public function handlePaymentNotification() {
        $url = CRM_Utils_System::url('civicrm/contribute/transact', "", FALSE, NULL, FALSE);
        $response = $_REQUEST;
        ...
        if (isset($response['hash2']) && $response['status'] == 100) {
            if (!CRM_Core_Payment_novalnet::novalnetCheckHash($response, $this->_password)) {
                ...
                $novalnetipn->main($response, 'novalnet_ideal');
            }
        } else {
