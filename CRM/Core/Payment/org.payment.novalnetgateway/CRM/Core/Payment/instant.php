 ...
 */
class CRM_Core_Payment_instant extends CRM_Core_Payment {
    ...
    function __construct($mode, &$paymentProcessor) {
        ...
        $this->_paymentType = 'novalnet_instant';
        ...
        $this->_testmode = trim(Civi::settings()->get('nn_instant_testmode'));
        defined('MODE_INSTANT') || define('MODE_INSTANT', $this->_testmode);
    ...
    static function &singleton($mode, &$paymentProcessor, &$paymentForm = NULL, $force = FALSE) {
		if (self::$_singleton[$paymentProcessor['name']] === NULL) {
            self::$_singleton[$paymentProcessor['name']] = new CRM_Core_Payment_instant($mode, $paymentProcessor);
        }
    ...
    public function handlePaymentNotification() {
        ...
        if (isset($response['hash2']) && $response['status'] == 100) {
            if (!CRM_Core_Payment_novalnet::novalnetCheckHash($response, $this->_password)) {
                ...
                $novalnetipn->main($response, 'novalnet_instant');
            }
        } else {
