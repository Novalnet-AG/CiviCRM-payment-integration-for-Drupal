...
/**
 * Payment Processor class for Novalnet Giropay
 */
class CRM_Core_Payment_giropay extends CRM_Core_Payment {
    ...
    function __construct($mode, &$paymentProcessor) {
        ...
        $this->_paymentType = 'novalnet_giropay';
		$this->_processorName = ts('Novalnet Giropay');
        ...
        $this->_testmode = trim(Civi::settings()->get('nn_giropay_testmode'));
        defined('MODE_GIROPAY') || define('MODE_GIROPAY', $this->_testmode);
    ...
    static function &singleton($mode, &$paymentProcessor, &$paymentForm = NULL, $force = FALSE) {
        if (self::$_singleton[$paymentProcessor['name']] === NULL) {
            self::$_singleton[$paymentProcessor['name']] = new CRM_Core_Payment_giropay($mode, $paymentProcessor);
        }
    ...
    public function handlePaymentNotification() {
        ...
        if (isset($response['hash2']) && $response['status'] == 100) {
            if (!CRM_Core_Payment_novalnet::novalnetCheckHash($response, $this->_password)) {
                ...
                $novalnetipn->main($response, 'novalnet_giropay');
            }
        } else {
