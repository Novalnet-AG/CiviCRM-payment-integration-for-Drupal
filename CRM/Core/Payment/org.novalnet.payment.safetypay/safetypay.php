...
#########################################################
#                                                       #
#  SafetyPay method class                               #
#  This module is used for real time processing of      #
#  SafetyPay payment of customers.                      #
#                                                       #
...
class org_novalnet_payment_safetypay extends CRM_Core_Payment {
  ...
  function __construct($mode, &$paymentProcessor) {
    ...
    $this->_processorName     = ts('Novalnet SafetyPay');
    $this->_paymentType       = 'novalnet_safetypay';
    ...
    $template->assign('novalnet_safetypay_name', trim($this->_paymentProcessor['name']));
    $template->assign('novalnet_safetypay_logo', $logo);
    $template->assign('novalnet_safetypay_desc', $description);
  }
  ...
  static function &singleton($mode, &$paymentProcessor, &$paymentForm = NULL, $force = FALSE) {
      ...
      if (self::$_singleton[$processorName] === NULL ) {
          self::$_singleton[$processorName] = new org_novalnet_payment_safetypay($mode, $paymentProcessor);
      }
   ...
   */
   function doTransferCheckout(&$params, $component) {
    ...
    $data['vendor_authcode']    = $this->_authcode;
    ...
    CRM_Core_Payment_novalnet::getPaymentReference($data, 'nn_safety');
    ...
    $_SESSION['novalnet']['completedata']['check']           = $data['check'];
    $_SESSION['novalnet']['completedata']['processor_name']  = $data['processor_name'];
  ...
  public function handlePaymentNotification() {
     ...
     $response  =  array_merge($_REQUEST,$_SESSION['novalnet']['completedata']);
